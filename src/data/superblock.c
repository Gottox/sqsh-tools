...
uint32_t
squash_data_superblock_modification_time(
...
uint32_t
squash_data_superblock_fragment_entry_count(
		const struct SquashSuperblock *superblock) {
	return le32toh(superblock->fragment_entry_count);
}

uint16_t
squash_data_superblock_compression_id(
		const struct SquashSuperblock *superblock) {
...
uint16_t
squash_data_superblock_version_major(
		const struct SquashSuperblock *superblock) {
	return le16toh(superblock->version_major);
}

uint16_t
squash_data_superblock_version_minor(
		const struct SquashSuperblock *superblock) {
	return le16toh(superblock->version_minor);
}

uint64_t
squash_data_superblock_root_inode_ref(
		const struct SquashSuperblock *superblock) {
...
uint64_t
squash_data_superblock_id_table_start(
		const struct SquashSuperblock *superblock) {
	return le64toh(superblock->id_table_start);
}

uint64_t
squash_data_superblock_xattr_id_table_start(
		const struct SquashSuperblock *superblock) {
...
uint64_t
squash_data_superblock_fragment_table_start(
		const struct SquashSuperblock *superblock) {
	return le64toh(superblock->fragment_table_start);
}

uint64_t
squash_data_superblock_export_table_start(
		const struct SquashSuperblock *superblock) {
